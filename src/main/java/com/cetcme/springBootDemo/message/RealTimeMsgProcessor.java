...
//            this.processMutilframe(message,length);
     ...
     * 原单帧数据处理
     */
    private void processSingleFrame(String deviceNo){
        String myDeviceNo = "";
        try{
            readMessageCount++;
            myDeviceNo = deviceNo;
...
//                responseTcpOk(message, message.getHeader());
            ...
            AcqData curData = new AcqData(); // todo: json 获取
...
//            responseTcpOk(message, message.getHeader());
            ...
            if (deviceExtend.getOuttimeFlag()) {
                ...
                logger.info("设备(" + deviceNo + ")解除离线超时警报结束");
            }
            ...
            if (deviceExtend.getOfflineFlag()) {
                ...
                deviceDao.setDeviceOnline(deviceNo);

                hsaDeviceStatusChange = true;
                logger.info("设备(" + deviceNo + ")上线设置结束");
            }

            if (hsaDeviceStatusChange) {
                RedissonUtil.redisson.getMap(RedisKey.DEVICE_INFO_CACHE.toString()).put( deviceNo, deviceExtend);
            }
